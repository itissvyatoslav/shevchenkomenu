...
//        for indexPush in 0..<Schedule.count{
        ...
        content.subtitle = TimeNames[0]
        content.body = "You have an hour before eating"
        content.sound = UNNotificationSound.default
        
        var date = DateComponents()
        let currentHour = Calendar.current.component(.hour, from: Schedule[0])
        let currentMinute = Calendar.current.component(.minute, from: Schedule[0])
        if currentHour == 0 {
            date.hour = 23
        } else {
